...
class RPC extends RPCBase {
    ...
    async _submitWork(data) {
        ...
        try {
            ...
            if (err.type === 'VerifyError') {
                this.logger.warning('RPC block rejected: %h (%s).', block.hash(), err.reason);
                return false;
            }
            throw err;
        }
    ...
    async deriveAddresses(args, help) {
        ...
        if (desc.isRange() && !valid.has(1)) {
            throw new RPCError(errs.INVALID_PARAMETER, 'Range must be specified for ranged descriptor');
        }
        if (!desc.isRange() && valid.has(1)) {
            throw new RPCError(errs.INVALID_PARAMETER, 'Range should not be specified for un-ranged descriptor');
        }
    ...
    getDescriptorRangeParams(valid) {
        ...
        try {
            ...
            try {
                ...
                throw new RPCError(errs.INVALID_PARAMETER, 'Invalid range.');
            }
        }
        if (!Number.isSafeInteger(low)) {
            throw new RPCError(errs.INVALID_PARAMETER, 'Range begin must be an integer');
        }
        if (!Number.isSafeInteger(high)) {
            throw new RPCError(errs.INVALID_PARAMETER, 'Range end must be an integer');
        }
        if (low > high) {
            throw new RPCError(errs.INVALID_PARAMETER, 'Range specified as [begin,end] must not have begin after end');
        }
        if (low < 0) {
...
function parseDescriptor(raw, network, requireChecksum = false) {
    try {
        ...
        throw new RPCError(errs.INVALID_DESCRIPTOR, `Invalid descriptor: ${e.message}`);
    }
}
