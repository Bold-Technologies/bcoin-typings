...
class KeyProvider {
    ...
    _parseKey(keyString, context) {
        ...
        if (keySplit.length === 1) {
            ...
            if (ring) {
                assert(permit || ring.publicKey.length === 33, 'Uncompressed keys are not allowed');
    ...
    parseKey(keyExpr, context) {
        ...
        assert(originSplit.length && originSplit[0][0] === '[', `Key origin start expected '[', found ${originSplit[0][0]} instead`);
...
class HDKeyProvider extends KeyProvider {
     ...
     */
    isHardened() {
        return (this.path.some(index => index & HD.common.HARDENED) ||
     ...
     */
    getHDPrivateKey(pos) {
        assert(this.hdkey.privateKey, 'Private key not available for hardened derivation.');
