...
class WSHDescriptor extends AbstractDescriptor {
    ...
    fromOptions(options) {
        ...
        assert(this.subdescriptors.length === 1, 'Must pass only 1 subdescriptor');
    ...
    fromString(str, network, context) {
        ...
        assert([scriptContext.TOP, scriptContext.P2SH].includes(context), 'Can only have wsh() at top level or inside sh()');
        str = strip(str);
