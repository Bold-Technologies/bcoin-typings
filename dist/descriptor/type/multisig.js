...
class MultisigDescriptor extends AbstractDescriptor {
    ...
    fromOptions(options) {
        ...
        assert((m & 0xff) === m && m > 0 && m <= MAX_MULTISIG_PUBKEYS, `Multisig threshold '${m}' is not valid`);
    ...
    fromString(str, network, context) {
        ...
        assert(valid.includes(context), 'Can only have multi/sortedmulti at top level, in sh(), or in wsh()');
        ...
        assert((m & 0xff) === m && m > 0 && m <= MAX_MULTISIG_PUBKEYS, `Multisig threshold '${threshold}' is not valid`);
        for (let i = 1; i < descArray.length; i++) {
            const provider = KeyProvider.fromString(descArray[i], network, context);
...
function isValidMultisig(m, n, providers, context = scriptContext.TOP) {
    ...
    assert(n && n <= MAX_MULTISIG_PUBKEYS, `Keys in multisig must be between 1-${MAX_MULTISIG_PUBKEYS} inclusive`);
    ...
    if (context === scriptContext.P2SH) {
        assert(scriptSize + 3 <= MAX_SCRIPT_PUSH, `P2SH script is too large (${scriptSize + 3} > ${MAX_SCRIPT_PUSH})`);
    }
    return true;
}
;
