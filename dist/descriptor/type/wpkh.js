...
class WPKHDescriptor extends AbstractDescriptor {
    ...
    fromOptions(options) {
        ...
        assert(this.keyProviders.length === 1, 'Can only have one key inside wpkh()');
        return this;
    }
    /**
    ...
    fromString(str, network, context) {
        ...
        assert([scriptContext.TOP, scriptContext.P2SH].includes(context), 'Can only have wpkh() at top level or inside sh()');
        str = strip(str);
