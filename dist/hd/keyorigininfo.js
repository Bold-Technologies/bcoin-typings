...
class KeyOriginInfo {
    ...
    fromOptions(options) {
        ...
        if (options.fingerPrint != null) {
            assert(options.fingerPrint >>> 0 === options.fingerPrint, 'Fingerprint must be uint32');
            this.fingerPrint = options.fingerPrint;
        }
        if (options.path != null) {
            if (Array.isArray(options.path)) {
                assert(options.path.every(p => p >>> 0 === p), 'All path indices must be uint32');
    ...
    fromString(str) {
        ...
        assert(slashSplit[0].length === 8, `Expected 8 characters fingerprint, found ${slashSplit[0].length} instead`);
     ...
     */
    equals(keyInfo) {
        if (!KeyOriginInfo.isKeyOriginInfo(keyInfo) ||
            this.fingerPrint !== keyInfo.fingerPrint ||
            this.path.length !== keyInfo.path.length) {
            return false;
        }
        return (this.path.every((p, i) => p === keyInfo.path[i]));
    }
    /**
    ...
    fromJSON(json) {
        if (json.fingerPrint) {
            if (typeof json.fingerPrint === 'string') {
                ...
                assert(json.fingerPrint >>> 0 === json.fingerPrint, 'Fingerprint must be uint32');
            }
            this.fingerPrint = json.fingerPrint;
        }
        if (json.path) {
            if (Array.isArray(json.path)) {
                assert(json.path.every(p => p >>> 0 === p), 'All path indices must be uint32');
                this.path = json.path;
            }
            else {
...
function validateFingerPrint(str) {
    assert(str.length === 8, `Expected 8 characters fingerprint, found ${str.length} instead`);
    const fingerPrint = parseInt(str, 16);
    assert(!isNaN(fingerPrint), `Fingerprint ${str} is not hex`);
    assert(fingerPrint >>> 0 === fingerPrint, 'Fingerprint must be uint32');
    return fingerPrint;
}
module.exports = KeyOriginInfo;
//# sourceMappingURL=keyorigininfo.js.map