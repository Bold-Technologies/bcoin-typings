...
class Chain extends AsyncEmitter {
    ...
    async updateInputs(block, prev) {
        ...
        for (let i = 1; i < block.txs.length; i++) {
            ...
            assert(await view.spendInputs(this.db, tx), 'BUG: Spent inputs in historical data!');
    ...
    async handleOrphans(entry) {
        ...
        while (orphan) {
            ...
            try {
                ...
                if (err.type === 'VerifyError') {
                    this.logger.warning('Could not resolve orphan block %h: %s.', block.hash(), err.message);
