...
/**
 * UTXO (Coin with additional properties)
 */
...
class CoinSelector {
  ...
  async init(coins) {
    ...
    for (let i = 0, n = coins.length; i < n; i++) {
      ...
      if (this.isSpendable(coin)) {
        const utxo = new UTXO();
        utxo.index = i;
        utxo.spendingSize = await coin.estimateSpendingSize(this.getAccount);
        utxo.effectiveValue = coin.value - this.getFee(utxo.spendingSize);
        this.utxos.push(utxo);
      }
    }
    this.utxos.sort((a, b) => b.effectiveValue - a.effectiveValue);
  ...
  async attemptSelection() {
    ...
    let size = await this.tx.estimateSize();
    const fee = this.getFee(size);
    target += fee;

    // try to find a changeless solution using BnB
    let result = await this.selectBnB(target, costOfChange);
    if (result && result.length > 0) {
      // we have found a changeless solution
      ...
      // update fee
      size = await this.tx.estimateSize();
      this.fee = this.getFee(size);
      return;
    }
    ...
    let bestSelection = [];
    let bestWaste = CoinSelector.MAX_MONEY;
    // ideally change value should be atleast 100 times of
    // cost of spending it in future. We will try to find such a solution.
    const usefulChange = 100 * costOfSpendingChange;
    ...
    // try to find a solution with useful change
    result = this.selectLowestLarger(target + usefulChange);
    if (result.length > 0) {
      const currWaste = this.getWaste(result, costOfChange,
        target + usefulChange);
      if (currWaste < bestWaste) {
        bestSelection = result;
        bestWaste = currWaste;
  ...
  selectSRD(target) {
    const coins = this.coins.map((coin, index) => {
      coin.pos = index;
      return coin;
    });
    ...
    while (index < coins.length) {
      ...
      selectedValue += coin.effectiveValue;
      selected.push(coin.pos);
  ...
  async selectBnB(target, costOfChange) {
    const selected = [];
    let currValue = 0;
    let totalValue = 0;
    let index = 0;
    for (const utxo of this.utxos)
      totalValue += utxo.effectiveValue;
    ...
    while (tries-- > 0) {
      ...
      // backtracking here
      if (backtrack) {
        // we have backtracked to the first UTXO,
        // all branches are traversed, we are done here
        if (selected.length === 0)
          break;

        // Add omitted UTXOs back before traversing
        // the omission branch of last included UTXO
        // we are using a while loop here beacuse
        // there can be more than one UTXOs that were omitted
        while (--index > selected[selected.length - 1]) {
          totalValue += this.utxos[index].effectiveValue;
        }

        // Remove last included UTXO from selected list.
        currValue -= this.utxos[index].effectiveValue;
        selected.pop();
      } else {
        ...
        if  (selected.length === 0
          ...
          || utxo.effectiveValue !== this.utxos[index - 1].effectiveValue) {
          selected.push(index);
          currValue += this.utxos[index].effectiveValue;
        }
      }
  ...
  selectLowestLarger(target) {
    ...
    this.index = 0;
    const selected = [];
    let effectiveValue = this.utxos[this.index].effectiveValue;
    while (target >= effectiveValue) {
      ...
      if (this.index === this.utxos.length)
        break;

      effectiveValue = this.utxos[this.index].effectiveValue;
    }

    if (target > 0 && this.index !== this.utxos.length) {
      ...
      const closestCoinIndex = this.findLowestLarger(target);
      target -= this.utxos[closestCoinIndex].effectiveValue;
      selected.push(closestCoinIndex);
    }
  ...
  findLowestLarger(target) {
    let i = this.index;
    let j = this.utxos.length - 1;
    // Corner cases
    // if target < smallest coin available
    // return the smallest coin
    if (target <= this.utxos[j].effectiveValue)
      return j;
    ...
    while (i < j) {
      ...
      const effectiveValue = this.utxos[mid].effectiveValue;

      if (effectiveValue === target)
        return mid;

      // if target is less than coin at mid
      // then search in left part of array
      if (target < effectiveValue) {
        if (mid < this.utxos.length - 1) {
          // calculate effective value of coin at mid + 1
          const nextEffectiveValue = this.utxos[mid + 1].effectiveValue;
          // if target is greater than next
          // effective value, then return mid
          // coin at mid > target > coin at mid + 1
          if (target > nextEffectiveValue)
            return mid;
        }
  ...
  /**
   * Calculate effectiveValue of a coin
   * effectiveValue = coin.value - cost of spending that coin
   * @param {Coin} coin - A coin object
   * @returns {Number} - effective value of the coin
   */

  async getEffectiveValue(coin) {
  ...
  getWaste(selected, costOfChange, target) {
    ...
    for (const i of selected) {
      ...
      selectedAmount += utxo.effectiveValue;
...
/**
 * Maximum possible supply of Bitcoin
 * @const {Amount}
 * @default
 */

CoinSelector.MAX_MONEY = 21000000 * 100000000;
